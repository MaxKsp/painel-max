...
final class AssistantActionExecutor {
    ...
    public function execute(int $userId, array $route): array {
        $action = $route['action'];
        ...
        return match ($action) {
            ...
            'create_workout_program' => $this->createWorkoutProgram($userId, $args),
            ...
            'create_diet_plan' => $this->createDietPlan($userId, $args),
    ...
    public function undo(int $userId, array $undo): string {
        ...
        if ($kind === 'diet_plan') {
            $previous = $undo['previous'] ?? null;
            $this->saveKvJson($userId, 'nutrition_plan_v1', is_array($previous) ? $previous : null);
            return 'Plano alimentar desfeito.';
        }
        if ($kind === 'workout_program') {
            $ids = is_array($undo['ids'] ?? null) ? $undo['ids'] : [];
            $deleted = 0;
            foreach ($ids as $id) {
                if (is_string($id) && training_delete_workout($this->db, $userId, $id)) $deleted++;
            }
            if ($deleted === 0) throw new RuntimeException('undo_conflict');
            return 'Programa de treino removido (' . $deleted . ' ficha(s)).';
        }
        if ($kind === 'measurement') {
    ...
    /** @param array<string,mixed> $args @return array<string,mixed> */
    private function createWorkoutProgram(int $uid, array $args): array {
        ...
        foreach (array_values($planned) as $item) {
            if (!is_array($item)) throw new InvalidArgumentException('Treino do programa inválido.');
            $workouts[] = training_save_workout($this->db, $uid, $item + ['id' => 'as_wo_' . substr(bin2hex(random_bytes(12)), 0, 24)], 'assistant');
        }
        $ids = array_map(static fn(array $w): string => (string)$w['id'], $workouts);
        $message = 'Programa criado: ' . count($workouts) . ' ficha(s) de treino para ' . $daysPerWeek . ' dia(s) por semana (' . $location . ').';
        return $this->mutationResult('create_workout_program', $message, 'treinos', ['kind' => 'workout_program', 'ids' => $ids], [
            'daysPerWeek' => $daysPerWeek, 'location' => $location, 'workouts' => $workouts,
        ]);
    }
    ...
    /** @param array<string,mixed> $args @return array<string,mixed> */
    private function createDietPlan(int $uid, array $args): array {
        ...
        $previous = $this->loadKvJson($uid, 'nutrition_plan_v1');
        $plan = [
            'goal' => $goal,
        ...
        $this->saveKvJson($uid, 'nutrition_plan_v1', $plan);
        $message = 'Plano alimentar criado: ' . $goal . ', ' . $periodDays . ' dia(s), custo estimado de R$ '
        ...
        return $this->mutationResult('create_diet_plan', $message, 'alimentacao', [
            'kind' => 'diet_plan', 'previous' => $previous,
        ], ['plan' => $plan]);
    }
